...
Page({
    ...
    loadUserDetail: function() {
        let _this = this;
        App._get('user.Index/getApplyDetail', {

        }, function(res) {
            _this.setData(res.data);
            _this.initPic(res.data);
        });
    },

    initPic: function(data) {
        let info = data.applyInfo;
        let _this = this;
        ...
        if (info.other.length > 0) {
            info.other.map(function(e, index) {
                _this.data.other_paths[index] = e.file_path;
                _this.data.other_ids[index] = e.file_id;
            })
            this.setData({
                other_paths: this.data.other_paths
            });
        }
    ...
    saveData: function(e) {
        ...
        values.company = this.data.company;
        values.other_content = this.data.other_content;
